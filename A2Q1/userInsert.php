...
if(isset($_POST['btnInsert'])){
    ...
    if($name != "" && $password != "" && $age != "" && $phone != ""){
        ...
        if(in_array($ext,$allowedExtImage) && $size < 3000000){
            ...
            echo "<script>alert('Photo File size should be less than 3000000 KB...')</script>";
        }
        else{
    ...
    if($imageUpload){
        $query = mysqli_query($con,"insert into eir_user(name,password,age,phone,photo,typeofuser) values('$name','$password',$age,'$phone','$newfilename','$typeofuser')");
        echo "<script>alert('Inserted Successfully..');</script>";
        if($typeofuser == 'Service Provider'){
...
?>
    ...
    <div class="container-fluid">
        ...
        <div class="row">
            <?php if($typeofuser == 'Service Provider'){ ?>
            <h1 class="title">Service Provider Insert</h1>
            <?php } elseif($typeofuser == 'Customer'){ ?>
